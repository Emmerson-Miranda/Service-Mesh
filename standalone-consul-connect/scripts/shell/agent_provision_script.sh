...
echo "ExecStart=/usr/local/bin/consul agent -data-dir=/tmp/consul -bind=$MYIP -enable-script-checks=true -config-dir=/etc/consul.d/ -join=172.20.20.10 " >> /etc/systemd/system/consul.service
